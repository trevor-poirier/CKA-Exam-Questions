if grep -q "advertise-address.endpoint: 172.30.1.2" /etc/kubernetes/manifests/kube-apiserver.yaml; then
    sed -i 's/advertise-address.endpoint: 172.30.1.2/advertise-address.endpoint: 127.0.0.1/g' /etc/kubernetes/manifests/kube-apiserver.yaml
else
...
if grep -q "advertise-address=172.30.1.2" /etc/kubernetes/manifests/kube-apiserver.yaml; then
    sed -i 's/advertise-address=172.30.1.2/advertise-address=127.0.0.1/g' /etc/kubernetes/manifests/kube-apiserver.yaml
else
